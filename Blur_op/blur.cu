...
void postProcess(const std::string& output_file, uchar4* data_ptr) {
  ...
  cv::imshow("gauss blur", imageOutputBGR);
  cv::waitKey(0);
  	//output the image
             ...
             float **h_filter, int *filterWidth, const std::string &filename)
{
    ...
    imageOutputRGBA.create(image.size(), CV_8UC4);
                              ...
                              const float* const filter, const int filterWidth)
{
    ...
    float color = 0.0f;
    for (int py=0; py < filterWidth; py++){
        for (int px=0; py < filterWidth; px++){
            int c_x = absolute_image_position_x + px - filterWidth / 2;
            int c_y = absolute_image_position_y + py - filterWidth / 2;
...
__global__ void separteChannels(const uchar4* const inputImageRGBA, int numRows, int numCols,
                                unsigned char* const redChannel,
                                unsigned char* const greenChannel,
                                unsigned char* const blueChannel)
{
...
int main(int argc, char** argv)
{
    ...
    separteChannels<<<gridSize, blockSize>>>(d_inputImageRGBA, numRows(), numCols(), d_red, d_green, d_blue);
